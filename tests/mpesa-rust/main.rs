...
mod mpesa_live_tests {
    ...
    impl MpesaClient {
        ...
        pub async fn b2c_payment<N: Into<f64>>(
            ...
            // TODO: after migration to v3 add OriginatorConversationID which is
            // generated by me the merchant to avoid double disbursement. can be
            // used to check the status of a transaction before making another
            // disbursement.
            self.inner
    ...
    async fn test_b2c_payment() {
        let client = get_test_client();
        let res = client
            .b2c_payment(client.config.msisdn(), 20.0, Some("Test"), None)
            .await;
